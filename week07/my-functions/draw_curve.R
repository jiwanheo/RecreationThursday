draw_curve <- function(start_x, 
                       ...
                       circles = NULL,     # circles here, are still in list form
                       collision_check,
                       retain_first) # Use the first angle encountered, until it ends. (ignore flow field)
  
{ 
  if(collision_check) {
    # Circle collision check #1
  ...
  # If the starting points are good, initialize the curve with the starting points, and keep generating
  ...
  # For passing around the dimensions to other functions (A bit of manual process.)
  x_dim_range <- 1:ncol(flow_field) 
  y_dim_range <- 1:nrow(flow_field)
  
  
  ##### CAN SAFELY IGNORE, if not retain_first.---------------------------------------------
  if(retain_first) { 
    first_angle <- step_into_next_curve_segment( 
      start_x = x_container[1], start_y = y_container[1], left_x = left_x, bot_y = bot_y, resolution = resolution,  x_dim_range = x_dim_range, y_dim_range = y_dim_range, flow_field = flow_field, step_length = step_length, 
      what_to_return = "angle"
    )              
  }                # If retain_first, Extract the first angle here,
  # and pass it to subsequent step_into_next_curve_segment.
  ####--------------------------------------------------------------------------------------
  ...
  for (i in 1:num_steps) {
    next_step <- step_into_next_curve_segment( 
      ...
      first_angle = if(retain_first){first_angle} else {NULL}
    )
